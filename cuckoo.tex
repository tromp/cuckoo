...
The ratio $\frac{M}{N}=1$ is suitable when a practically guaranteed solution is desired.
...
This works whenever $\frac{M}{N} \leq \frac{1}{2}$ since the expected degree of a node is then at most 1,
...
To the extent that this slowdown is unavoidable, it can be called the memory hardness of the proof-of-work.
...
we did find an imporoved BFS($L/2$) TMTO algorithm (implemented in momentomatum.cpp)
...
BFS($L/2$) TMTO with an additional safet factor of 2.
...
interval time. This desirable proprty is known as {\em progress-freeness}, and in our case
...
has steadily climbed and is currently at 67,
representing an incredible $2^{67}/10$ double-hashes per minute.
...
that, by requiring a non-trivial amounts of memory, resist such
