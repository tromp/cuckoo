...
based on finding cycles or other structures in large random graphs.
...
As we will see in section~\ref{cycle_length_choice}, Momentum is in essence a special case of Cuckoo Cycle.
...
inserting numbers in a Cuckoo hashtable naturally leads to cycle formation
...
plus about 64KB per thread for 2 path-following arrays.
...
\begin{center}
\begin{tikzpicture}
\begin{axis}[xlabel={$\frac{E}{N}$ in \%}, ylabel={\% runtime}, legend pos=north west]
\addplot[color=blue] coordinates {
...
The ratio $\frac{E}{N}$ determines a base level of difficulty,
which may suffice for applications where difficulty is to remain fixed.
The ratio $\frac{E}{N}=1$ is suitable when a practically guaranteed solution is desired.
For crypto currencies, where difficulty must scale in precisely
controlled manner across a huge range, adjusting the number of edges is not suitable.
The implementation default $\frac{E}{N}=\frac{1}{2}$ gives a solution probability of roughly $2.2\%$,
...
This works whenever $\frac{E}{N} \leq \frac{1}{2}$ since the expected degree of a node is then at most 1,
and a constant fraction of edges are expected to be leaf edges.
...
These steps are repeated for the other partition of odd nodes.
Preprocessor symbol PART\_BITS, whose value we'll denote as $B$,
allows for trading-off node counter storage for runtime,
by processing the nodes in multiple passes depending on the value of their $B$ least significant bits
(apart from the very least significant bit which partitions the nodes into even and odd ones).
The memory usage is $M$ bits for the alive set and $N / 2^{B}$ for the counters.

The diagrams below show two rounds of edge trimming on the earlier example. In round one
...
It selects a suitably small subset $Z$ of even vertices as a base layer,
...
in at least 50 of the 200 graphs. Below we plot the slowdown relative to the reference algorithm also using 4MB
...
The zig-zagging is caused by the current implementation being limited to 2-power sizes of
...
with cycle length, while the BFS($L$) algorithm exhibits roughly linear slowdown.
...
This is in essence Cuckoo Cycle with $N=2^25+2^25$ nodes, $M=2^26$ edges and cycle length $L=2$, the difference
being that edges are generated not by equation~\ref{hash_edges}, but by splitting a SHA512 hash of
$(k,\mbox{nonce} / 8)$ into 8 64-bit words, taking the most significant 50 bits of the
$\mbox{nonce} \bmod 8)-th$ one, and viewing that as a pair of two 25-bit numbers to which we can append
a 0 resp. 1 bit to make them even resp. odd.
Since this is such a special case, there is likely to be a greater variety of algorithms that are more
efficient than for the general case. While we haven't found a improved main algorithm, we did find
an imporoved TMTO algorithm, that is very similar to the BFS($L/2$) one but cuts the memory usage in half,
resulting in a slowdown of only $1.75$---a lack of memory-hardness.

The preceding analysis suggests that larger cycle length improves memory hardness.
...
to the bitmap of live edges, each thread will handle words (of 32 edge-bits each) spaced spaces apart by
...
at 67, representing an incredible $2^{67}/10$ double-hashes per minute.
...
Downsides of this development include rapid obsolescence,
large power consumption, and centralization of mining power.
...
memory latency dominates the runtime.
This narrows the mining performance gap between custom and commodity hardware and
could lead to mining costs being dominated by investments in DRAM.
