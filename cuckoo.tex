...
We propose to base proofs-of-work on finding certain subgraphs in large random graphs.
...
Each key $k \in \{0,1\}^K$ generates a graph $G_k = (V,E)$ where $V=\{0,\ldots,N-1\}$, and
\begin{equation}
E=\{(h(k,2i) \bmod N,h(k,2i+1) \bmod N) | i \in [0,\ldots,M-1]\}
\end{equation}
The graph has a {\em solution} if $H$ occurs as a subgraph.
Denote the number of edges in h as $|H|$.
A proof of solution is an ordered list of $|H|$ numbers that generate the edges of $H$'s occurance in $G$.
Such a proof is verifiable in constant time, independent of $N$ and $M$.

A simple variation generates random bipartite graphs:
\begin{equation}
E=\{(h(k,2i) \bmod \frac{N}{2}, \frac{N}{2} + (h(k,2i+1) \bmod \frac{N}{2}) | i \in [0,\ldots,M-1]\}
\end{equation}
...
It follows that the hash based random bipartite graph construction of our
proof-of-work scheme matches the underlying structure of Cuckoo hashing.
