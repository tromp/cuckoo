...
There is a trade-off in the other direction, at least on certain platforms.
By precomputing all the siphashes (with option -p) in a big array, total runtime
may be reduced by more than 20\%, despite the extra memory accesses. We currently
lack a good explanation of this behaviour.
...
In the first approach we reduce the number of rounds in the \hash function,
shown in red and blue.
% checking that results are still random enough so as not to affect the distribution of cycle lengths.
In the second approach we precompute all the hashes (doubling the memory use) in the main program,
and then only count the runtime of running the threads using the precomputed edges,
as shown in orange.
