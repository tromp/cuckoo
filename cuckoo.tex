...
incurs an $\Omega(k)$ slowdown where the constant grows with cycle length
(roughly 25 at the suggested cycle length of 42).
We hypothesize that this extra order of magnitude slowdown is unavoidable.
\end{abstract}
...
It selects a suitably small subset $Z$ of vertices from partition $U$, and builds a BFS forest of depth
equal to cycle length $L$. For each new BFS layer, it enumerates all edges to see which ones are incident
