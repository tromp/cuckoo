...
while \cite{poelstra2014} argues the virtues of Bitcoin's choice of PoW.
...
%\footnote{and with dynamic sizing, will continue to do so into the distant future.}.
This narrows the mining performance gap between custom and commodity hardware and
could lead to mining costs being dominated by upfront investments in DRAM rather than ongoing power costs.
Just as SRAM is one order of magnitude faster but 2 orders more expensive than DRAM,
and as such a big net loss for Cuckoo Cycle throughput per dollar,
so it is conceivable that development and production of custom cuckoo memory chips
will incur a big enough cost premium to wipe out any performance gains.
