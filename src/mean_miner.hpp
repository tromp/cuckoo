...
const static u32 BIGHASHMASK    = (1<<BIGHASHBITS) - 1;
...
static const u32 SENTINELSIZE = NSIPHASH * 5; // sentinel for bigsort1;
...
// store        edge    degree  smallbkt
      ...
      z = node & BUCKETMASK;
      u64 zz = (u64)block << BIGHASHBITS | (node & EDGEMASK) >> BUCKETBITS;
#ifndef NEEDSYNC
      *(u64 *)(big0+big[z]) = zz;
      big[z] += BIG0SIZE;
#else
