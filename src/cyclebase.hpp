...
struct cyclebase {
  ...
  void alloc() {
    cuckoo = (node_t *)calloc(NCUCKOO, sizeof(node_t));
  ...
  void resetcounts() {
    memset(cuckoo, -1, NCUCKOO * sizeof(node_t)); // for prevcycle nil
  ...
  void cycles() {
    int len, len2;
    node_t us2[MAXPATHLEN], vs2[MAXPATHLEN];
    for (int i=0; i < ncycles; i++) {
      node_t u = cycleedges[i].u, v = cycleedges[i].v;
      int   nu = path(u, us),    nv = path(v, vs);
      ...
      do  {
        ...
        node_t u2 = cycleedges[i2].u, v2 = cycleedges[i2].v;
        ...
        if (us[nu] == us2[nu2]) {
          ...
          if (len + len2 > 0) {
#if 0
            node_t ubranch = us[nu-len], vbranch = vs[nv-len2];
