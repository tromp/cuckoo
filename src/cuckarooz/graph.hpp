...
// cuck(ar)ooz graph with given limit on number of edges (and on single partition nodes)
...
public:
  ...
  graph(word_t maxedges, word_t maxnodes, u32 maxsols, u32 compressbits, char *bytes) : visited(maxnodes) {
    ...
    links   = new (bytes += sizeof(word_t[MAXNODES])) link[2*MAXEDGES];
    compress = compressbits ? new compressor<word_t>(EDGEBITS, compressbits, bytes += sizeof(link[2*MAXEDGES])) : 0;
    sharedmem = true;
  ...
  uint64_t bytes() {
    return sizeof(word_t[MAXNODES]) + sizeof(link[2*MAXEDGES]) + (compress ? compress->bytes() : 0);
  }
  ...
  bool add_compress_edge(word_t u, word_t v) {
    return add_edge(compress->compress(u), compress->compress(v));
  }
};
