...
public:
  ...
  edgetrimmer(const u32 n_blocks, const u32 tpb, const u32 n_trims, const bool show_all) {
    ...
    checkCudaErrors(cudaMalloc((void**)&tedges, sizeof(zbucket<TBUCKETSIZE,0,0,0>[NY])));
#endif
  ...
  __device__ void genUnodes(const u32 uorv) {
    ...
    const u32 starty = NY *  blockIdx.x    / nblocks;
...
// write        edge     YYYYYY    ZZZZZ
    ...
    if (!blockIdx.x) printf("genUnodes round %2d size %u\n", uorv, sumsize/BIGSIZE0);
  ...
  __device__ void genVnodes(const u32 uorv) {
    ...
    const u32 startux = NX *  blockIdx.x    / nblocks;
...
#if BIGSIZE0==4
          BIGTYPE e = *(u32 *)readbig;
#else
          BIGTYPE e = read40(readbig);
#endif
#if defined(NEEDSYNC)
...
// write         edge     UZZZZ   within UX UY partition
          write40(small0+small.index[uy], ((u64)edge << ZBITS) | (e & ZMASK));;
...
// if (blockIdx.x==0) printf("id %d ux %d uy %d e %010lx pref %4x edge %x mask %x\n", blockIdx.x, ux, uy, e, e>>ZBITS, edge, NONDEGMASK);
        ...
        const u16 *readz = tzs[blockIdx.x];
...
// write      UYYYYY    UZZZZZ     VYYYYY     VZZZZ   within VX partition
          write40(base+dst.index[vx], uy34 | ((u64)*readz << YZBITS) | (node & YZMASK));
...
// write      UXXXXX    UYYYYY     UZZZZZ     VZZZZ   within VX VY partition
          write40(small0+small.index[vy], ((u64)uxyz << ZBITS) | (e & ZMASK));
          uxyz &= ~ZMASK;
...
// write     VYYZZZ"     UYYZZ'   within UX partition
  ...
  __device__ void recoveredges() {
    ...
    for (readedges += threadIdx.x; readedges < endreadedges; readedges += blockDim.x) {
      u32 edge = *readedges;
      if (dipnode(sip_keys, edge, 1) == v && dipnode(sip_keys, edge, 0) == u) {
        sol[blockIdx.x] = edge;
      }
    }
#endif
  ...
  __device__ void recoveredges1() {
    ...
    if (threadIdx.x < PROOFSIZE) {
      const u32 uxy = uvnodes[2*threadIdx.x] >> ZBITS;
...
public:
  ...
  u32 threadbytes() const {
    return sizeof(thread_ctx) + sizeof(zbucket<TBUCKETSIZE,0,0,0>[NY]) + sizeof(zbucket8) + sizeof(zbucket16) + sizeof(zbucket32);
  }
...
#ifdef SAVEEDGES
    _recoveredges<<<PROOFSIZE,1024>>>(trimmer->dt);
#else
